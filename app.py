...
def _review_chapter_summaries(rows: list[dict[str, Any]] | None) -> list[dict[str, Any]]:
    ...
    for item in grouped.values():
        ...
        item["progress_label"] = f"{completed}/{total}" if total > 0 else "0/0"
...
def _render_review_workspace(base_url: str, actions_key: str) -> None:
    ...
    if chapter_summaries:
        ...
        st.dataframe(
            ...
            hide_index=True,
            width="stretch",
            column_order=["title", "status_label", "progress_label", "completion_rate", "completed", "pending_high", "pending", "all", "high", "selected", "replacement_ready"],
            column_config={
                "title": st.column_config.TextColumn("章节", width="medium"),
                "status_label": st.column_config.TextColumn("状态", width="small"),
                "progress_label": st.column_config.TextColumn("进度", width="small"),
                "completion_rate": st.column_config.TextColumn("完成率", width="small"),
                "completed": st.column_config.NumberColumn("已完成", width="small"),
