...
from backend.zhifei_autoplan.phase2_final_review_issue_list import (  # noqa: E402
    PASS_STATUS,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build and validate the no-runtime Phase 2E final review issue list."
    )
    ...
    snapshot = build_phase2e_final_review_issue_list_snapshot(
        args.root,
        fixture_path=args.fixture,
    )
    if args.json:
        print(dump_phase2e_final_review_issue_list_json(snapshot))
    else:
        print(format_phase2e_final_review_issue_list_report(snapshot))
