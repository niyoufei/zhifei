...
from backend.zhifei_autoplan.phase2_risk_object_binding import (  # noqa: E402
    PASS_STATUS,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build and validate the no-runtime Phase 2C risk-object binding."
    )
    ...
    snapshot = build_phase2c_risk_object_binding_snapshot(
        args.root,
        fixture_path=args.fixture,
    )
    if args.json:
        print(dump_phase2c_risk_object_binding_json(snapshot))
    else:
        print(format_phase2c_risk_object_binding_report(snapshot))
