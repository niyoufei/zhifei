...
from backend.zhifei_autoplan.phase2_output_pre_index import (  # noqa: E402
    PASS_STATUS,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build and validate the no-runtime Phase 2F output pre-index."
    )
    ...
    snapshot = build_phase2f_output_pre_index_snapshot(
        args.root,
        fixture_path=args.fixture,
    )
    if args.json:
        print(dump_phase2f_output_pre_index_json(snapshot))
    else:
        print(format_phase2f_output_pre_index_report(snapshot))
