...
def _build_docx(base: Path, data: Dict[str, Any], src_path: Path, title_prefix: str, style: Dict[str, Any] | None = None) -> Dict[str, str]:
    ...
    # 审计追溯索引（与 compose 同步）
    try:
        trace["tender_matrix_loaded"] = bool(_load_tender_matrix())
    except Exception:
        trace["tender_matrix_loaded"] = None
    try:
        ak = _get_active_kg()
        trace["active_kg_file"] = ak.get("file_name") if ak else None
        trace["active_kg_sha256"] = ak.get("sha256") if ak else None
    except Exception:
        trace["active_kg_file"] = None
        trace["active_kg_sha256"] = None
    ...
    meta = {
        ...
        "trace_chain": {
            "tender_matrix_loaded": trace.get("tender_matrix_loaded"),
            "active_kg_file": trace.get("active_kg_file"),
            "active_kg_sha256": trace.get("active_kg_sha256"),
        },
        "style": {
