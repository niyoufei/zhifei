...
@pytest.mark.asyncio
async def test_actions_export_docx_delegates_to_service(tmp_path):
    ...
    with patch.object(ws, "WORKSPACE_ROOT", tmp_path / "workspaces"), patch.dict(
        ...
        clear=False,
    ), patch.object(actions_bridge.export_docx_core, "execute_export_docx_request", side_effect=_fake_execute_export_docx_request):
        response = await actions_bridge.actions_export_docx(
            req,
            session_id="router-export",
    ...
    assert str(seen["workspace_dir"]).endswith("router-export")
    assert callable(seen["save_outputs_fn"])


@pytest.mark.asyncio
async def test_actions_export_docx_preserves_reference_library_media_fields(tmp_path):
    from backend.app.routers import actions_bridge
    from backend.zhifei_autoplan import workspace as ws
