...
@pytest.mark.asyncio
async def test_actions_case_library_upload_and_items_roundtrip(tmp_path):
...
@pytest.mark.asyncio
async def test_actions_case_library_upload_rejects_invalid_project_type(tmp_path):
    ...
    with patch.object(ws, "WORKSPACE_ROOT", workspaces), patch.dict(
        ...
        with pytest.raises(HTTPException) as exc:
            await actions_case_library_upload(
                ...
                title="无效案例",
                template_page_bucket="50_pages",
                session_id="case-lib-invalid",
                x_actions_key="test-actions-key",
            )

    assert exc.value.status_code == 400
    assert exc.value.detail == "template library upload requires valid project_type"


def test_build_case_reference_pack_no_match_warns(tmp_path):
...
def test_build_case_reference_pack_with_selected_case_ids(tmp_path):
    from backend.zhifei_autoplan.case_library_service import build_case_reference_pack
    from backend.zhifei_autoplan.template_library import build_template_chapter_profiles, template_library_record_id
    ...
    record = {
        ...
        "template_chapter_profiles": build_template_chapter_profiles(extract_path.read_text(encoding="utf-8")),
        "sha256": "a" * 64,
    ...
    pack = build_case_reference_pack(
        options={"enabled": True, "selected_case_ids": [template_library_record_id(record)]},
        topic="医院改造",
        chapter_title="工程概况",
        project_type="房建",
        template_page_bucket="50_pages",
    ...
    assert pack["selected_case_ids"] == [template_library_record_id(record)]
    assert pack["match_reason"] == "selected_case_ids"
    assert pack["style_hints"]


@pytest.mark.asyncio
async def test_actions_image_library_upload_and_items_roundtrip(tmp_path):
...
@pytest.mark.asyncio
async def test_actions_image_library_upload_rejects_invalid_project_type(tmp_path):
...
def test_build_image_selection_pack_roundtrip(tmp_path):
...
def test_build_image_selection_pack_no_match_is_noop_warning(tmp_path):
