...
IMAGE_LIBRARY_TAG = "image_library"
...
def is_image_library_record(rec: dict[str, Any] | None) -> bool:
    if not isinstance(rec, dict):
        return False
    return str(rec.get("library_scope") or "").strip() == IMAGE_LIBRARY_SCOPE
...
def _record_public_item(rec: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "description": str(rec.get("library_description") or rec.get("library_note") or "").strip(),
...
def summarize_image_selection_pack(raw: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "selected_image_ids": [
            str(x).strip()
            for x in (pack.get("selected_image_ids") or [])
            if str(x).strip()
        ],
...
def image_selection_pack_media_entries(raw: Any) -> list[dict[str, Any]]:
    ...
    for item in pack.get("images") or []:
        if not isinstance(item, dict):
            continue
        path = str(item.get("source_path") or item.get("storage_path") or "").strip()
        if not path:
            continue
