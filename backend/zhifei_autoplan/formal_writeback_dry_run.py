...
REQUIRED_FORMAL_WRITEBACK_DRY_RUN_FIELDS = frozenset(
    {
        "contract_version",
        "dry_run_id",
        "request_id",
...
CURRENT_STAGE_EMITTABLE_DRY_RUN_STATUSES = frozenset(
    {
        "not_created",
        "blocked",
        "stale_source_hash",
...
DRY_RUN_TARGET_TYPES = frozenset(
    {
        "source_section",
        "section_draft",
        "docx_document",
...
def _formal_writeback_dry_run_id(seed: dict[str, Any]) -> str:
...
def build_formal_writeback_dry_run(
    *,
    request_id: str,
    ...
    dry_run_scope: str = "metadata_only",
    dry_run_mode: str = "disabled_current_stage",
    dry_run_target_type: str = "metadata_only",
    dry_run_request_status: str = "not_requested",
    dry_run_requested: bool = False,
    dry_run_payload_hash: str = "",
    dry_run_candidate_hash: str = "",
    dry_run_source_snapshot_hash: str = "",
    ...
    dry_run_request_triggered: bool = False,
    dry_run_id: str = "",
    dry_run_status: str = "",
) -> dict[str, Any]:
    ...
    dry_run_decision = _text(dry_run_decision, limit=120)
    dry_run_scope = _text(dry_run_scope, limit=120)
    dry_run_mode = _text(dry_run_mode, limit=120)
    dry_run_target_type = _text(dry_run_target_type, limit=120)
    dry_run_request_status = _text(dry_run_request_status, limit=120)
    dry_run_payload_hash = _text(dry_run_payload_hash, limit=240)
    dry_run_candidate_hash = _text(dry_run_candidate_hash, limit=240)
    dry_run_source_snapshot_hash = _text(dry_run_source_snapshot_hash, limit=240)
    ...
    zbid_isolation_status = _text(zbid_isolation_status, limit=120)
    ...
    requested_status = _text(dry_run_status, limit=120)
    dry_run_id = _text(dry_run_id, limit=240)

    dry_run_requested = _bool(dry_run_requested)
    ...
    dry_run_request_triggered = _bool(dry_run_request_triggered)
    ...
    enum_checks = (
        (requested_status, DRY_RUN_STATUSES, "invalid_dry_run_status"),
    ...
    missing_id_reasons = {
        "missing_shadow_candidate_id": shadow_candidate_id,
        "missing_patch_id": patch_id,
        "missing_approval_id": approval_id,
        "missing_diff_preview_id": diff_preview_id,
        "missing_rollback_plan_id": rollback_plan_id,
        "missing_writeback_guard_id": writeback_guard_id,
        "missing_source_hash_guard_id": source_hash_guard_id,
        "missing_review_apply_guard_id": review_apply_guard_id,
        "missing_docx_isolation_guard_id": docx_isolation_guard_id,
        "missing_zbid_isolation_guard_id": zbid_isolation_guard_id,
    }
    for reason, value in missing_id_reasons.items():
        if not value:
            _append_unique(blocked_reasons, reason)
    ...
    if zbid_isolation_status in {
        "blocked",
        "not_created",
        "stale_source_hash",
        "stale_source_version",
    }:
        _append_unique(blocked_reasons, "zbid_isolation_guard_not_ready")

    if response_mode == "thinking_only_fallback":
        _append_unique(blocked_reasons, "thinking_only_fallback_not_dry_run_capable")
    ...
    if dry_run_request_status in request_status_blocks:
        _append_unique(blocked_reasons, request_status_blocks[dry_run_request_status])

    if dry_run_requested:
        _append_unique(blocked_reasons, "dry_run_request_blocked")
    if not dry_run_payload_hash:
        _append_unique(blocked_reasons, "dry_run_payload_blocked")
    if not dry_run_candidate_hash:
        _append_unique(blocked_reasons, "missing_dry_run_candidate_hash")
    if not dry_run_source_snapshot_hash:
        _append_unique(blocked_reasons, "missing_dry_run_source_snapshot_hash")
    ...
    request_blocks = (
        ...
        ("dry_run_request_triggered_blocked", dry_run_request_triggered),
    )
    for reason, requested in request_blocks:
        if requested:
            _append_unique(blocked_reasons, reason)

    if requested_status == "passed_shadow_only" or dry_run_decision == "pass_shadow_only":
        _append_unique(blocked_reasons, "dry_run_is_not_formal_writeback_permission")

    _append_unique(
        blocked_reasons, "real_formal_writeback_dry_run_not_implemented_current_stage"
    )
    ...
    dry_run_seed = {
        ...
        "dry_run_payload_hash": dry_run_payload_hash,
        "dry_run_candidate_hash": dry_run_candidate_hash,
        "dry_run_source_snapshot_hash": dry_run_source_snapshot_hash,
        "writeback_candidate_hash": writeback_candidate_hash,
        "docx_candidate_hash": docx_candidate_hash,
        "zbid_candidate_hash": zbid_candidate_hash,
        "zbid_target_mapping_hash": zbid_target_mapping_hash,
    ...
    if not dry_run_id:
        dry_run_id = _formal_writeback_dry_run_id(dry_run_seed)
    ...
    return {
        ...
        "dry_run_target_type": dry_run_target_type,
        "dry_run_request_status": dry_run_request_status,
        ...
        "zbid_isolation_status": zbid_isolation_status,
        ...
        "dry_run_request_triggered": dry_run_request_triggered,
    }
