"""Pure ReviewInputV2 boundary for Route C.

The V2 record is a canonical, reference-only projection of one verified
ReviewInputV1.  It does not make recommendations, emit review events, or
perform workflow, approval, or feedback-loop work.
"""
...
from .review import (
    DiagnosticResultSetReferenceV1,
    ReviewInputProvenanceV1,
    ReviewInputV1,
...
class SourceReviewInputReferenceV1:
    """The complete identity reference to the verified V1 source record."""
    ...
    def __post_init__(self) -> None:
        if self.artifact_type != ReviewInputV1.ARTIFACT_TYPE:
            raise CanonicalError("SCHEMA_VIOLATION", "source artifact_type must be review-input")
        if self.schema_version != ReviewInputV1.SCHEMA_VERSION:
            raise CanonicalError("SCHEMA_VIOLATION", "source schema_version must be 1")
        stable_id = normalize_text(self.stable_id)
        revision_id = normalize_text(self.revision_id)
        stable_prefix = f"ocrc:{ReviewInputV1.ARTIFACT_TYPE}:"
        revision_prefix = f"ocrc-rev:{ReviewInputV1.ARTIFACT_TYPE}:"
    ...
    def from_verified_review_input(
        cls, source: ReviewInputV1
    ) -> "SourceReviewInputReferenceV1":
        return cls(
...
@dataclass(frozen=True, slots=True)
class ReviewInputV2:
    """Canonical V2 review input derived only from a verified V1 record."""
    ...
    source_review_input: SourceReviewInputReferenceV1
    ...
    @classmethod
    def from_verified_review_input(cls, source: ReviewInputV1) -> "ReviewInputV2":
        """Construct V2 without accepting any caller-controlled overrides."""
        ...
        source_reference = SourceReviewInputReferenceV1.from_verified_review_input(source)
        content = cls._content_projection(
            source.target,
            source.diagnostic_result_set,
            source_reference,
        )
        provenance_projection = source.provenance.to_projection()
