...
REQUIRED_ZBID_ISOLATION_GUARD_FIELDS = frozenset(
    {
        "contract_version",
...
ZBID_ISOLATION_STATUSES = frozenset(
    {
        "not_created",
        "blocked",
...
CURRENT_STAGE_EMITTABLE_ZBID_ISOLATION_STATUSES = frozenset(
    {
        "not_created",
        "blocked",
        "stale_source_hash",
...
ZBID_WRITEBACK_DECISIONS = frozenset(
    {
        "none",
        "block",
        "isolate_shadow_only",
...
ZBID_WRITEBACK_SCOPES = frozenset(
    {
        "single_section",
        "selected_sections",
        "full_document",
...
ZBID_WRITEBACK_MODES = frozenset(
    {
...
ZBID_WRITEBACK_REQUEST_STATUSES = frozenset(
    {
        "not_requested",
        "requested_blocked",
        "route_blocked",
...
def _zbid_isolation_guard_id(seed: dict[str, Any]) -> str:
    payload = json.dumps(seed, ensure_ascii=False, sort_keys=True, separators=(",", ":"))
    digest = hashlib.sha256(payload.encode("utf-8")).hexdigest()
    return f"zbid-isolation-guard-{digest[:16]}"


def build_zbid_isolation_guard(
    *,
    request_id: str,
    ...
    review_apply_guard_id: str = "",
    docx_isolation_guard_id: str = "",
    zbid_writeback_decision: str = "none",
    zbid_writeback_scope: str = "metadata_only",
    zbid_writeback_mode: str = "disabled_current_stage",
    zbid_target_type: str = "metadata_only",
    zbid_writeback_request_status: str = "not_requested",
    zbid_writeback_requested: bool = False,
    zbid_writeback_route: str = "",
    zbid_writeback_payload_hash: str = "",
    zbid_candidate_hash: str = "",
    zbid_target_mapping_hash: str = "",
    zbid_source_snapshot_hash: str = "",
    docx_candidate_hash: str = "",
    ...
    source_hash_guard_status: str = "",
    review_apply_isolation_status: str = "",
    docx_isolation_status: str = "",
    source_hash_revalidation_status: str = "",
    source_version_revalidation_status: str = "",
    ...
    review_apply_request_triggered: bool = False,
    export_docx_request_triggered: bool = False,
    zbid_writeback_request_triggered: bool = False,
    zbid_isolation_guard_id: str = "",
    zbid_isolation_status: str = "",
) -> dict[str, Any]:
    ...
    review_apply_guard_id = _text(review_apply_guard_id, limit=240)
    docx_isolation_guard_id = _text(docx_isolation_guard_id, limit=240)
    zbid_writeback_decision = _text(zbid_writeback_decision, limit=120)
    zbid_writeback_scope = _text(zbid_writeback_scope, limit=120)
    zbid_writeback_mode = _text(zbid_writeback_mode, limit=120)
    zbid_target_type = _text(zbid_target_type, limit=120)
    zbid_writeback_request_status = _text(zbid_writeback_request_status, limit=120)
    zbid_writeback_route = _text(zbid_writeback_route, limit=240)
    zbid_writeback_payload_hash = _text(zbid_writeback_payload_hash, limit=240)
    zbid_candidate_hash = _text(zbid_candidate_hash, limit=240)
    zbid_target_mapping_hash = _text(zbid_target_mapping_hash, limit=240)
    zbid_source_snapshot_hash = _text(zbid_source_snapshot_hash, limit=240)
    docx_candidate_hash = _text(docx_candidate_hash, limit=240)
    ...
    source_hash_guard_status = _text(source_hash_guard_status, limit=120)
    review_apply_isolation_status = _text(review_apply_isolation_status, limit=120)
    docx_isolation_status = _text(docx_isolation_status, limit=120)
    ...
    requested_status = _text(zbid_isolation_status, limit=120)
    zbid_isolation_guard_id = _text(zbid_isolation_guard_id, limit=240)
    ...
    review_apply_request_triggered = _bool(review_apply_request_triggered)
    export_docx_request_triggered = _bool(export_docx_request_triggered)
    zbid_writeback_request_triggered = _bool(zbid_writeback_request_triggered)
    ...
    enum_checks = (
        (requested_status, ZBID_ISOLATION_STATUSES, "invalid_zbid_isolation_status"),
        (zbid_writeback_decision, ZBID_WRITEBACK_DECISIONS, "invalid_zbid_writeback_decision"),
        (zbid_writeback_scope, ZBID_WRITEBACK_SCOPES, "invalid_zbid_writeback_scope"),
        (zbid_writeback_mode, ZBID_WRITEBACK_MODES, "invalid_zbid_writeback_mode"),
        (zbid_target_type, ZBID_TARGET_TYPES, "invalid_zbid_target_type"),
        (
            zbid_writeback_request_status,
            ZBID_WRITEBACK_REQUEST_STATUSES,
            "invalid_zbid_writeback_request_status",
        ),
    ...
    if not source_hash_guard_id:
        _append_unique(blocked_reasons, "missing_source_hash_guard_id")
    if not review_apply_guard_id:
        _append_unique(blocked_reasons, "missing_review_apply_guard_id")
    if not docx_isolation_guard_id:
        _append_unique(blocked_reasons, "missing_docx_isolation_guard_id")
    ...
    if source_hash_guard_status in {
        "blocked",
        "not_created",
        "stale_source_hash",
        "stale_source_version",
    }:
        _append_unique(blocked_reasons, "source_hash_guard_not_ready")
    if review_apply_isolation_status in {
        "blocked",
        "not_created",
        "stale_source_hash",
        "stale_source_version",
    }:
        _append_unique(blocked_reasons, "review_apply_guard_not_ready")
    if docx_isolation_status in {
        "blocked",
        "not_created",
        "stale_source_hash",
        "stale_source_version",
    }:
        _append_unique(blocked_reasons, "docx_isolation_guard_not_ready")

    if response_mode == "thinking_only_fallback":
        _append_unique(blocked_reasons, "thinking_only_fallback_not_zbid_writeback_capable")
    ...
    if source_hash_revalidation_status == "mismatched":
    ...
    if source_version_revalidation_status == "mismatched":
    ...
    request_status_blocks = {
        "requested_blocked": "zbid_writeback_request_blocked",
        "route_blocked": "zbid_writeback_route_blocked",
        "payload_blocked": "zbid_writeback_payload_blocked",
        "mapping_blocked": "zbid_target_mapping_blocked",
    }
    if zbid_writeback_request_status in request_status_blocks:
        _append_unique(blocked_reasons, request_status_blocks[zbid_writeback_request_status])

    if zbid_writeback_requested:
        _append_unique(blocked_reasons, "zbid_writeback_request_blocked")
    if zbid_writeback_route:
        _append_unique(blocked_reasons, "zbid_writeback_route_blocked")
    if not zbid_writeback_payload_hash:
        _append_unique(blocked_reasons, "zbid_writeback_payload_blocked")
    if not zbid_candidate_hash:
        _append_unique(blocked_reasons, "missing_zbid_candidate_hash")
    if not zbid_target_mapping_hash:
        _append_unique(blocked_reasons, "missing_zbid_target_mapping_hash")
        _append_unique(blocked_reasons, "zbid_target_mapping_blocked")
    if not zbid_source_snapshot_hash:
        _append_unique(blocked_reasons, "missing_zbid_source_snapshot_hash")

    required_hashes = {
        "docx_candidate_hash": docx_candidate_hash,
    ...
    request_blocks = {
        ...
        "review_apply_request_blocked": review_apply_request_triggered,
        "export_docx_request_blocked": export_docx_request_triggered,
        "zbid_writeback_request_blocked": zbid_writeback_request_triggered,
    }
    for reason, requested in request_blocks.items():
        if requested:
            _append_unique(blocked_reasons, reason)

    if requested_status == "isolated_shadow_only" or zbid_writeback_decision == (
        "isolate_shadow_only"
    ):
        _append_unique(blocked_reasons, "zbid_isolation_is_not_writeback_permission")

    _append_unique(blocked_reasons, "real_zbid_isolation_not_implemented_current_stage")
    ...
    zbid_guard_seed = {
        ...
        "review_apply_guard_id": review_apply_guard_id,
        "docx_isolation_guard_id": docx_isolation_guard_id,
        "zbid_writeback_payload_hash": zbid_writeback_payload_hash,
        "zbid_candidate_hash": zbid_candidate_hash,
        "zbid_target_mapping_hash": zbid_target_mapping_hash,
        "zbid_source_snapshot_hash": zbid_source_snapshot_hash,
        "docx_candidate_hash": docx_candidate_hash,
        "writeback_candidate_hash": writeback_candidate_hash,
        "source_snapshot_hash": source_snapshot_hash,
        "before_text_hash": before_text_hash,
    ...
    if not zbid_isolation_guard_id:
        zbid_isolation_guard_id = _zbid_isolation_guard_id(zbid_guard_seed)

    return {
        "contract_version": CONTRACT_VERSION,
        "zbid_isolation_guard_id": zbid_isolation_guard_id,
        "request_id": request_id,
        "source_document_id": source_document_id,
        "source_section_id": source_section_id,
        "source_section_hash": source_section_hash,
        ...
        "review_apply_guard_id": review_apply_guard_id,
        "docx_isolation_guard_id": docx_isolation_guard_id,
        "zbid_isolation_status": status,
        "zbid_writeback_decision": zbid_writeback_decision,
        "zbid_writeback_scope": zbid_writeback_scope,
        "zbid_writeback_mode": zbid_writeback_mode,
        "zbid_target_type": zbid_target_type,
        "zbid_writeback_request_status": zbid_writeback_request_status,
        "zbid_writeback_requested": zbid_writeback_requested,
        "zbid_writeback_route": zbid_writeback_route,
        "zbid_writeback_payload_hash": zbid_writeback_payload_hash,
        "zbid_candidate_hash": zbid_candidate_hash,
        "zbid_target_mapping_hash": zbid_target_mapping_hash,
        "zbid_source_snapshot_hash": zbid_source_snapshot_hash,
        ...
        "source_hash_guard_status": source_hash_guard_status,
        "review_apply_isolation_status": review_apply_isolation_status,
        "docx_isolation_status": docx_isolation_status,
        ...
        "review_apply_request_triggered": review_apply_request_triggered,
        "export_docx_request_triggered": export_docx_request_triggered,
        "zbid_writeback_request_triggered": zbid_writeback_request_triggered,
    }
