...
REQUIRED_REVIEW_APPLY_ISOLATION_GUARD_FIELDS = frozenset(
    {
        "contract_version",
...
REVIEW_APPLY_ISOLATION_STATUSES = frozenset(
    {
        "not_created",
        "blocked",
...
CURRENT_STAGE_EMITTABLE_REVIEW_APPLY_ISOLATION_STATUSES = frozenset(
    {
        "not_created",
        "blocked",
        "stale_source_hash",
...
REVIEW_APPLY_DECISIONS = frozenset(
    {
        "none",
        "block",
        "isolate_shadow_only",
...
REVIEW_APPLY_TARGET_TYPES = frozenset(
    {
        "source_section",
        "section_draft",
        "patch_preview",
...
REVIEW_APPLY_REQUEST_STATUSES = frozenset(
    {
        "not_requested",
        "requested_blocked",
        "route_blocked",
...
def build_review_apply_isolation_guard(
    *,
    ...
    review_apply_decision: str = "none",
    review_apply_scope: str = "metadata_only",
    review_apply_mode: str = "disabled_current_stage",
    review_apply_target_type: str = "metadata_only",
    review_apply_request_status: str = "not_requested",
    review_apply_requested: bool = False,
    review_apply_route: str = "",
    review_apply_payload_hash: str = "",
    ...
    review_apply_decision = _text(review_apply_decision, limit=120)
    review_apply_scope = _text(review_apply_scope, limit=120)
    review_apply_mode = _text(review_apply_mode, limit=120)
    review_apply_target_type = _text(review_apply_target_type, limit=120)
    review_apply_request_status = _text(review_apply_request_status, limit=120)
    review_apply_route = _text(review_apply_route, limit=240)
    review_apply_payload_hash = _text(review_apply_payload_hash, limit=240)
    ...
    requested_status = _text(review_apply_isolation_status, limit=120)
    ...
    enum_checks = (
        (requested_status, REVIEW_APPLY_ISOLATION_STATUSES, "invalid_review_apply_isolation_status"),
        ...
        (review_apply_target_type, REVIEW_APPLY_TARGET_TYPES, "invalid_review_apply_target_type"),
        (review_apply_request_status, REVIEW_APPLY_REQUEST_STATUSES, "invalid_review_apply_request_status"),
    ...
    if response_mode == "thinking_only_fallback":
        _append_unique(blocked_reasons, "thinking_only_fallback_not_review_apply_capable")
    ...
    if review_apply_request_status in request_status_blocks:
        _append_unique(blocked_reasons, request_status_blocks[review_apply_request_status])

    if review_apply_requested:
        _append_unique(blocked_reasons, "review_apply_request_blocked")
    if review_apply_route == "/review/apply":
        _append_unique(blocked_reasons, "review_apply_route_blocked")
    if not review_apply_payload_hash:
        _append_unique(blocked_reasons, "review_apply_payload_blocked")
    ...
    if requested_status == "isolated_shadow_only" or review_apply_decision == "isolate_shadow_only":
        _append_unique(blocked_reasons, "review_apply_isolation_is_not_writeback_permission")

    _append_unique(blocked_reasons, "real_review_apply_isolation_not_implemented_current_stage")

    if status not in {"not_created", "stale_source_hash", "stale_source_version"}:
        status = "blocked"

    review_apply_guard_seed = {
        ...
        "review_apply_payload_hash": review_apply_payload_hash,
        "writeback_candidate_hash": writeback_candidate_hash,
        "source_snapshot_hash": source_snapshot_hash,
        "before_text_hash": before_text_hash,
        "after_text_preview_hash": after_text_preview_hash,
    ...
    if not review_apply_guard_id:
        review_apply_guard_id = _review_apply_guard_id(review_apply_guard_seed)
    ...
    return {
        ...
        "writeback_guard_id": writeback_guard_id,
        "source_hash_guard_id": source_hash_guard_id,
        "review_apply_isolation_status": status,
        "review_apply_decision": review_apply_decision,
        "review_apply_scope": review_apply_scope,
        "review_apply_mode": review_apply_mode,
        "review_apply_target_type": review_apply_target_type,
        "review_apply_request_status": review_apply_request_status,
        "review_apply_requested": review_apply_requested,
        "review_apply_route": review_apply_route,
        "review_apply_payload_hash": review_apply_payload_hash,
        "writeback_candidate_hash": writeback_candidate_hash,
        "source_snapshot_hash": source_snapshot_hash,
        "before_text_hash": before_text_hash,
